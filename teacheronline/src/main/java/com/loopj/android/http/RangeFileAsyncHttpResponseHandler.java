    ...
    public void sendResponseMessage(HttpResponse response) throws IOException {
        if (!Thread.currentThread().isInterrupted()) {
            ...
            if (status.getStatusCode() == HttpStatus.SC_REQUESTED_RANGE_NOT_SATISFIABLE) {
                ...
                if (!Thread.currentThread().isInterrupted())
                    sendFailureMessage(status.getStatusCode(), response.getAllHeaders(), null, new HttpResponseException(status.getStatusCode(), status
                            .getReasonPhrase()));
            } else {
                if (!Thread.currentThread().isInterrupted()) {
            ...
            if (instream != null) {
                try {
                    ...
                    while (current < contentLength && (l = instream.read(tmp)) != -1 && !Thread.currentThread().isInterrupted()) {
                        ...
                        //delete by lijinzhong sendProgressMessage((int) current, (int) contentLength);
                    }
