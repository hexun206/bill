...
package com.huatu.teacheronline.widget.linecharview;
 ...
 * 绘制折线图处Y坐标轴之外的其他元素
 *
 * @author ljyu 22017-4-28 14:05:56
 */
public class LineView extends View {
    ...
    private ArrayList<Point> circlePoint;//当前数据圆点的坐标 集合
    private int selectIndex = 1; // 选中的圆点 默认为1
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        selectIndex = lineParameters.getDrawMap().size();
...
//            canvas.drawText(value + "", x - (paint.measureText(value + "")), newY - 10, paint);
        ...
        for (int i = 0; i < circlePoint.size(); i++) {
            Point point = circlePoint.get(i);
            paint.setStrokeWidth(2);
            paint.setStyle(Paint.Style.FILL);
            paint.setColor(Color.parseColor("#59ccb2"));
            ...
            if (i == selectIndex - 1) {
                 ...
                 */
                paint.setStrokeWidth(defaultStrokeWidth);
                paint.setTextSize(dpToPx(lineParameters.getLineVaueTextSize()));
                paint.setColor(lineParameters.getLineColor());
                double value = Double.parseDouble(lineParameters.getDrawMap().get("x_" + i));
                canvas.drawText(value + "", point.x - 40, point.y - 30, paint);
            }
            //设置为0 ，暂时隐藏
